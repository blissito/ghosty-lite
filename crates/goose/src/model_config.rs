...
fn materialize_model_config_inner(
    ...
    model = model.with_default_max_tokens(config.get_ghosty_max_tokens()?);
    ...
    if model.cache_ttl().is_none() {
        if let Some(ttl) = get_ghosty_cache_ttl(config)? {
...
pub fn with_rederived_cache_ttl(model: ModelConfig) -> Result<ModelConfig> {
    ...
    if let Some(ttl) = get_ghosty_cache_ttl(Config::global())? {
        model = model.with_cache_ttl(&ttl);
    }
    Ok(model)
}

fn get_ghosty_cache_ttl(config: &Config) -> Result<Option<String>> {
    match config.get_param::<String>("GHOSTY_CACHE_TTL") {
        Ok(ttl) => {
            let ttl = ttl.trim().to_lowercase();
            match ttl.as_str() {
                "5m" | "1h" => Ok(Some(ttl)),
                other => Err(anyhow!(
                    "GHOSTY_CACHE_TTL must be '5m' or '1h', got '{other}'"
                )),
            }
        }
...
mod cache_ttl_tests {
    ...
    fn env_var_populates_cache_ttl() {
        let _guard = env_lock::lock_env([("GHOSTY_CACHE_TTL", Some("1h"))]);
    ...
    fn absent_env_var_leaves_cache_ttl_unset() {
        let _guard = env_lock::lock_env([("GHOSTY_CACHE_TTL", None::<&str>)]);
        let model = materialize_model_config_inner(
            ModelConfig::new("claude-sonnet-4-5"),
            "anthropic",
            false,
    ...
    fn invalid_env_var_is_rejected() {
        let _guard = env_lock::lock_env([("GHOSTY_CACHE_TTL", Some("2h"))]);

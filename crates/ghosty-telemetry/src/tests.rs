...
//!
//! One of them is load-bearing beyond ordinary coverage:
...
use crate::buffer;
use crate::counters::KNOWN_PROVIDERS;
use crate::decision::{
    EndpointError, Resolver, TelemetryDecision, TelemetryInputs, decide, permission_still_enabled,
...
/// The resolved-switch half of [`TelemetryInputs`]: what the host's config
/// layer would hand over after applying every persistent and run-scoped source.
#[derive(Debug, Clone)]
struct Opts {
    enabled: bool,
    explicit_off: bool,
    /// `None` is the dry-run sink here, matching the shape the original
    /// resolver produced from an explicitly empty `telemetry_endpoint`.
    endpoint: Option<String>,
}

fn resolved(telemetry: bool, explicit_off: bool, endpoint: Option<&str>) -> Opts {
    Opts {
        enabled: telemetry,
        explicit_off,
...
/// The notice half: `true` means the user declined at some version.
type Setup = bool;
...
fn declined_setup() -> Setup {
    true
}

/// Accepted an older notice version. A disclosure refresh is not a decline.
fn stale_setup() -> Setup {
    false
}

fn inputs_for(home: Option<&Path>, opts: &Opts, declined: Setup) -> TelemetryInputs {
    TelemetryInputs {
        home: home.map(Path::to_path_buf),
        enabled: opts.enabled,
        explicit_off: opts.explicit_off,
        notice_declined: declined,
        endpoint: Some(opts.endpoint.clone().unwrap_or_default()),
        config_path: None,
    }
}

fn resolver_for(home: Option<&Path>, opts: &Opts, declined: Setup) -> Resolver {
    let inputs = inputs_for(home, opts, declined);
    Arc::new(move || inputs.clone())
}

/// Resolve the predicate against fixed facts, the way every surface does after
/// its own config layer has run.
fn decide_in_home(
    home: Option<&Path>,
    opts: &Opts,
    setup: &Setup,
    ...
    decide(resolver_for(home, opts, *setup), surface)
}

/// A resolver that re-reads a one-line config file on every call, so the
/// tests can model "another process wrote `telemetry = false`".
fn file_backed_resolver(home: &Path, config_path: &Path) -> Resolver {
    let home = home.to_path_buf();
    let config_path = config_path.to_path_buf();
    Arc::new(move || {
        let body = std::fs::read_to_string(&config_path).unwrap_or_default();
        let enabled = !body.contains("telemetry = false");
        TelemetryInputs {
            home: Some(home.clone()),
            enabled,
            explicit_off: !enabled,
...
#[test]
fn flush_redecision_fails_closed_when_the_resolver_cannot_read() {
    // A resolver whose sources are unreadable must answer "off, but nobody
    // revoked anything". That shape is `ForcedOff`: the flush is suppressed
    // and nothing on disk is touched.
    let home = temp_home();
    let root = root_of(&home);
    buffer::ensure_dir(&root).expect("create root");
    let before = snapshot(&root);
    let unreadable: Resolver = Arc::new(|| TelemetryInputs {
        enabled: false,
        explicit_off: false,
        ..TelemetryInputs::default()
    });
    assert!(matches!(
        re_decide(unreadable, Surface::Exec),
        TelemetryDecision::ForcedOff
    ));
    assert_eq!(snapshot(&root), before, "a failed re-decision touched disk");
}
...
fn closed_enum_values() -> Vec<String> {
    ...
    // `providers` entries: `KNOWN_PROVIDERS` is a closed list of
    // `&'static str`, and an unknown name yields the literal "other".
    values.extend(KNOWN_PROVIDERS.iter().map(|name| (*name).to_string()));
    values
}
...
fn hostile_buffer_lines_are_dropped_before_they_reach_a_batch() {
    let hostile = [
        ...
        // `record_provider` maps through `KNOWN_PROVIDERS` to avoid.
...
fn every_legitimately_recorded_event_survives_the_drain() {
    ...
    // Everything `record_provider` can ever store must be a legal `providers`
    // entry, or a real user's `session_end` would be dropped at the drain.
    for name in KNOWN_PROVIDERS {
        assert!(
            crate::event::is_known_provider_id(name),
            "a recordable provider is not a legal `providers` entry: {name}"
        );
        assert_eq!(crate::counters::canonical_provider(name), *name);
    }
    assert_eq!(
        crate::counters::canonical_provider("acme_internal_gateway"),
        "other"
    );
    assert!(!crate::event::is_known_provider_id("acme_internal_gateway"));
    assert!(
        !crate::event::is_known_provider_id("Anthropic"),
        "the drain bound is an exact match; only `record_provider` normalizes"
    );
}
...
fn git_sha_is_null_without_release_env() {
    // `GHOSTY_RELEASE_BUILD_SHA` is read at compile time and only release CI
    // sets it, so on a developer machine this is `None` and on release CI it
    // is twelve hex characters. Both are asserted, because the test has to
    // pass in both places and neither shape may ever be a path, a version, or
    // a full sha.
...
fn an_unparseable_env_value_forces_off_and_does_not_wipe() {
    // The host's config floor turns an unreadable `GHOSTY_TELEMETRY` into
    // `enabled == false` *without* setting `explicit_off`. A
...
fn only_opt_out_touches_disk() {
    ...
    let forced_off_rows: Vec<(Opts, Setup)> = vec![
...
fn install_id_is_random_and_rotates() {
    ...
    envelope::atomic_write_json(&buffer::install_id_path(&first_root), &stale)
...
fn stale_consent_cannot_clear_a_newer_opt_out_but_fresh_reenable_can() {
    ...
    let live = file_backed_resolver(home.path(), &config_path);
    ...
    // tombstone generation — cannot arm, because arm re-runs the resolver
    // while holding the wipe lock.
    ...
    assert!(
        buffer::arm(&root, stale_consent.tombstone_generation(), || {
            permission_still_enabled(&live, &root)
        })
        .is_err(),
    ...
    let TelemetryDecision::Enabled(fresh_consent) = decide(live.clone(), Surface::Tui) else {
        panic!("fresh re-enable must produce consent");
    };
    assert_eq!(fresh_consent.config_path(), Some(config_path.as_path()));
    buffer::arm(&root, fresh_consent.tombstone_generation(), || {
        permission_still_enabled(&fresh_consent.resolver(), &root)
    })
    .expect("fresh re-enable arms");
    assert!(!buffer::tombstone_present(&root));

    // A re-decision through the consent's own resolver sees the live file:
    // this is what the flush path relies on.
    std::fs::write(&config_path, "telemetry = false\n").expect("persist opt-out again");
    assert!(matches!(
        re_decide(fresh_consent.resolver(), Surface::Tui),
        TelemetryDecision::OptedOut
    ));
    assert!(buffer::tombstone_present(&root));
}
...
#[test]
fn unknown_provider_emits_literal_other() {
    let counters = crate::SessionCounters::default();
    counters.record_provider("acme_internal_gateway");
    counters.record_provider("DeepSeek");
    counters.record_provider("/Users/victim/providers.toml");
    counters.record_provider("deepseek");
    let providers = counters.providers();
    assert_eq!(providers, vec!["deepseek".to_string(), "other".to_string()]);
    for name in &providers {
        assert!(crate::event::is_known_provider_id(name));
    }
}

#[test]
fn every_known_provider_satisfies_the_server_shape_rule() {
    // `^[a-z0-9]([a-z0-9-]{0,30}[a-z0-9])?$`, sorted, deduplicated, and
    // at most 32 entries — the ingest service's rules for `providers`.
    assert!(KNOWN_PROVIDERS.len() <= 32);
    for pair in KNOWN_PROVIDERS.windows(2) {
        assert!(pair[0] < pair[1], "KNOWN_PROVIDERS is not sorted: {pair:?}");
    }
    for name in KNOWN_PROVIDERS {
        let bytes = name.as_bytes();
        assert!(!bytes.is_empty() && bytes.len() <= 32, "{name}");
        assert!(
...
fn the_notice_summarizes_what_the_schema_collects_and_states_every_red_line() {
    ...
    assert_eq!(notice::NOTICE_HEADLINE, "¿Ayudas a mejorar Ghosty?");
    for line in body.lines() {
        assert!(
            line.chars().count() <= 72,
            "the notice must wrap at 72 columns: {line:?}"
        );
    }

    // The modal names the useful product categories and links the exact
    // field-by-field schema. `install_id` is "un ID aleatorio guardado en
    // esta máquina"; transport metadata remains in the linked document. The
    // body wraps at 72 columns, so multi-word claims are matched across the
    // reflowed whitespace.
    let flat: String = body.split_whitespace().collect();
    for claim in [
        "versión",
        "sistema operativo y de CPU",
        "duración y el desenlace de la sesión",
        "contadores agregados de funciones y de errores",
        "ID aleatorio guardado en esta máquina",
        "cada 90 días",
    ] {
        assert!(
    ...
    for red_line in [
        ...
        "prompts",
        "archivos",
        "nombres de repositorio o de rama",
        "contenido del modelo",
        "credenciales",
        "línea de tiempo por turno o por herramienta",
    ] {
        assert!(
    ...
    assert!(!lowered.contains("anonimiza"));
    assert!(!lowered.contains("anonymized"));

    // The modal names the persistent opt-out, which is the switch that also
    // fulfils its deletion promise, and the run-only kill switch — labelled
    // as run-only, because it erases nothing.
    assert!(body.contains("ghosty configure → Ajustes → Telemetría"));

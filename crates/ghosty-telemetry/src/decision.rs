...
//!
//! This crate does not read config files itself. The host application supplies
//! a [`Resolver`] — a closure that re-reads its own config, setup state and
//! environment from scratch and returns [`TelemetryInputs`]. The consent token
//! carries that closure so the flush path and the arming path can re-run it
//! and observe a mid-session opt-out written by another process.
...
/// must build this **from scratch on every call** — never from a cached value —
/// because it is re-run at flush time precisely to observe changes made by
/// another process.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct TelemetryInputs {
    /// The Ghosty home directory. `None` when no home can be resolved: there
    /// is nowhere to keep an install id or a buffer, so the decision is
    /// `ForcedOff`, never a wipe.
    pub home: Option<PathBuf>,
    /// The resolved value of the telemetry switch after every persistent and
    /// run-scoped source was applied.
    pub enabled: bool,
    /// Whether a `false` in `enabled` came from **durable** config (a human
    /// persistently said no). A run-scoped `GHOSTY_TELEMETRY=0` or an
    /// unparseable value resolves `enabled == false` with this left `false`.
...
/// Re-reads the host's configuration from disk and environment.
///
/// An `Arc<dyn Fn>` rather than a bare `fn()` so a host (or a test) can close
/// over a config path or a temporary home. It must be `Send + Sync` because the
/// writer thread re-runs it before every flush.
pub type Resolver = Arc<dyn Fn() -> TelemetryInputs + Send + Sync>;
...
impl fmt::Debug for TelemetryConsent {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TelemetryConsent")
            .field("root", &self.root)
            .field("endpoint", &self.endpoint)
            .field("surface", &self.surface)
            .field("config_path", &self.config_path)
            .field("tombstone_generation", &self.tombstone_generation)
...
impl TelemetryConsent {
    ...
    /// The resolver this consent was produced by, so the flush path can
    /// re-run it.
    #[must_use]
...
/// Resolve the emit predicate.
///
/// Runs `resolver` once and classifies the result. The predicate, in order:
///
...
#[must_use]
pub fn decide(resolver: Resolver, surface: Surface) -> TelemetryDecision {
    let inputs = resolver();
...
fn evaluate(inputs: &TelemetryInputs) -> TelemetryEvaluation {
    let root = inputs.home.as_deref().map(|home| home.join(TELEMETRY_DIR));
...
/// token may arm only when the resolver's fresh answer, home, and endpoint
/// still classify as enabled for the same root.
pub(crate) fn permission_still_enabled(resolver: &Resolver, expected_root: &Path) -> bool {
    let inputs = resolver();
    matches!(
        evaluate(&inputs),
...
///
/// Runs the same resolver the process was armed with, so a
/// `telemetry = false` written by another process between init and flush is
/// honoured. A resolver that cannot read its sources must answer with
/// `enabled: false, explicit_off: false` — a flush is never the right place to
/// guess, and that shape is `ForcedOff`, never a wipe.
#[must_use]
pub fn re_decide(resolver: Resolver, surface: Surface) -> TelemetryDecision {
    decide(resolver, surface)
}

...
pub enum Event {
    ...
    SessionEnd {
        ...
        /// Sorted, deduplicated [`crate::counters::KNOWN_PROVIDERS`] entries. An unknown provider yields
...
impl Event {
    ...
    /// struct of `u32`s, `providers` comes from `KNOWN_PROVIDERS`, and
...
///
/// Checked against [`crate::counters::KNOWN_PROVIDERS`], the same closed list
/// `record_provider` maps every name through. Exact match only: the drain
/// path uses this as a bound, so a lowercase-and-retry here would let a
/// planted buffer line through that this process never wrote.
#[must_use]
pub fn is_known_provider_id(value: &str) -> bool {
    crate::counters::KNOWN_PROVIDERS.contains(&value)
}

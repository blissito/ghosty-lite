...
///
/// Sourced from `GHOSTY_RELEASE_BUILD_SHA`, an environment variable that
/// release CI sets **at compile time** of this crate (`option_env!`) and that
/// no developer machine sets. `null` for every locally built binary,
...
///
/// Serializes to a sibling temp file in the same directory, flushes and
/// fsyncs it, restricts it to `0600` on Unix, and renames it over `path`. A
/// reader therefore sees either the previous file or the complete new one,
/// never a torn write.

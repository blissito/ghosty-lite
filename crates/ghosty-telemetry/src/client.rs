...
///
/// The client is a plain `reqwest::blocking::Client` built on the `rustls`
/// feature, which installs its own crypto provider: a build that skipped the
/// provider would silently never connect on some platforms — indistinguishable
/// from fail-open, which means no test that merely asserts "does not crash"
/// would catch it.
///
...
fn post(endpoint: &str, app_version: &str, body: String) -> SendOutcome {
    let client = reqwest::blocking::Client::builder()
        ...
        // No cookie store exists to disable: `reqwest` is built here without
        // the `cookies` feature, so there is no jar to carry state between
        // batches even if a server tried to set one.

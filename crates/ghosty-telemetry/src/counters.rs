...
/// The closed list of provider ids this crate will ever put on the wire.
///
/// Every entry satisfies the ingest server's shape rule
/// (`^[a-z0-9]([a-z0-9-]{0,30}[a-z0-9])?$`), and the list is sorted so the
/// server's sorted-and-deduplicated rule holds by construction. Anything not
/// in this list is recorded as the literal `"other"` — a customer's own
/// `[providers.<name>]` table key never reaches a payload.
...
/// The bucket every unknown provider name collapses into.
pub const OTHER_PROVIDER: &str = "other";

/// Map a free-form provider name onto [`KNOWN_PROVIDERS`].
///
/// Case-insensitive; a name outside the list yields [`OTHER_PROVIDER`]. The
/// return is a `&'static str` from the list, never a slice of the input.
#[must_use]
pub fn canonical_provider(name: &str) -> &'static str {
    let lowered = name.trim().to_ascii_lowercase();
    KNOWN_PROVIDERS
        .iter()
        .copied()
        .find(|known| *known == lowered)
        .unwrap_or(OTHER_PROVIDER)
}
...
pub struct SessionCounters {
    ...
    /// Sorted, deduplicated entries of [`KNOWN_PROVIDERS`]. `&'static str`
...
impl SessionCounters {
    ...
    ///
    /// The name is mapped through [`canonical_provider`] **before** it is
    /// stored: only a `&'static str` from [`KNOWN_PROVIDERS`] is ever kept, so
    /// a customer's own `[providers.<name>]` table key collapses to `"other"`
    /// and cannot reach the wire.

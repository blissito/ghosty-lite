//! Deterministic, Rust-driven orchestration for `ghosty review`.
//!
...
//!
//! - One subprocess per check (`ghosty run -q -t <prompt>`)
...
/// Run all discovered checks concurrently as `ghosty run` subprocesses.
///
...
pub async fn run_checks_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, check, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("ghosty review: check task panicked: {e}");
                continue;
            }
        };
        ...
        match result {
            Ok(findings) => {
                if !quiet {
                    eprintln!(
                        "ghosty review: check '{}' completed: {} finding(s)",
                        check.name,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("ghosty review: check '{}' failed: {e}", check.name);
                results[idx] = Vec::new();
            }
        }
    }
...
///
/// Uses `ghosty review --turn-limit` when set, otherwise
...
/// Spawn a single `ghosty run` subprocess for one check and parse its
...
/// Generic `ghosty run` subprocess that hands a prompt to the model
...
async fn run_subprocess_for_findings(
    ...
    let goose_bin = std::env::current_exe().context("locate current ghosty binary")?;
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        // Closing stdin signals EOF to `ghosty run -i -`.
        drop(stdin);
    }
...
pub async fn run_main_pass_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, path, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("ghosty review: main-pass task panicked: {e}");
                continue;
            }
        };
        match result {
            Ok(raw) => {
                ...
                if !quiet {
                    eprintln!(
                        "ghosty review: main pass on '{}' completed: {} finding(s)",
                        path,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("ghosty review: main pass on '{}' failed: {e}", path);
...
/// cap enforced by ghosty. Without this, models routinely burn turns on

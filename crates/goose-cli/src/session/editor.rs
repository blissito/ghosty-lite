...
fn build_template(messages: &[&str], prefill: Option<&str>) -> String {
    let mut content = String::from("# Ghosty · redacta tu mensaje\n\n");
...
mod tests {
    ...
    fn test_extract_user_input_with_editor_output() {
        let content = r#"# Ghosty · redacta tu mensaje
...
"#;
    ...
    fn test_create_temp_file_with_messages() {
        ...
        assert!(content.contains("# Ghosty · redacta tu mensaje"));
...
"#;
    ...
    fn test_editor_uses_random_tempfile_and_preserves_cwd() {
        ...
        fs::write(
            ...
            r#"printf '%s' "$4" > "$1"
pwd > "$2"
printf '# Ghosty · redacta tu mensaje\n\n# Your prompt:\n\nupdated prompt\n' > "$4"
